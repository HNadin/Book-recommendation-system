...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--sentiments-csv", type=str, default="sentiments.csv",
            help="Path to pre-calculated sentiments CSV generated by sentiment_clean.py"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        sentiments_path = options.get("sentiments_csv", "sentiments.csv")
        if os.path.exists(sentiments_path):
            try:
                self.stdout.write(f"  Loading pre-calculated sentiments from {sentiments_path} …")
                # Завантажуємо ваш згенерований файл
                sent_df = pd.read_csv(sentiments_path, low_memory=False)

                # Миттєво групуємо мільйон відгуків за ISBN та знаходимо середній сентимент (compound)
                sentiment_map = sent_df.groupby("final_isbn")["polarity"].mean().to_dict()

                n_matched = len(sentiment_map)
                if n_matched:
                    avg = sum(sentiment_map.values()) / n_matched
                    # Застосовуємо корегування до тренувального набору (ratings_df)
                    train_df = apply_sentiment_correction(train_df, sentiment_map)
                    self.stdout.write(
                        f"  Matched {n_matched:,} books via pre-calculated sentiments "
                        f"(avg compound: {avg:+.3f})."
                    )
                else:
                    self.stdout.write(self.style.WARNING("  sentiments.csv produced no matches."))
            except Exception as e:
                self.stdout.write(self.style.WARNING(f"  Sentiment correction failed: {e}"))
        else:
            self.stdout.write(self.style.WARNING(
                f"  File {sentiments_path} not found — sentiment correction skipped."
            ))

        # Save sentiment map for the live recommender
