...
def _enrich_books_with_stats(isbn_list: list) -> list:
    ...
    for isbn in isbn_list:
        ...
        result.append({
            ...
            "bx_count": b.n,
            "goodreads": _get_goodreads(isbn) or (
                {"rating": round(b.goodreads_rating, 2)} if b.goodreads_rating else None
            ),
        })
    return result
        ...
        search_results = [
            {
                ...
                "bx_rating": round(b.avg, 1) if b.avg else None,
                "bx_count": b.n,
                "goodreads": _get_goodreads(b.isbn) or (
                    {"rating": round(b.goodreads_rating, 2)} if b.goodreads_rating else None
                ),
    ...
    if isbn:
        ...
        if book_qs:
            selected = {
                ...
                "bx_count": book_qs.n,
                "goodreads": _get_goodreads(isbn) or (
                    {"rating": round(book_qs.goodreads_rating, 2)} if book_qs.goodreads_rating else None
                ),
                ...
                neighbours = [
                    {
                        ...
                        "goodreads": _get_goodreads(nb_isbn) or (
                            {"rating": round(nb_books[nb_isbn].goodreads_rating, 2)}
                            if nb_books[nb_isbn].goodreads_rating else None
                        ),
